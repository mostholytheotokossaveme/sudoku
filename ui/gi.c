...
void gi_on_kbd(void* g, unsigned int k) {
  if(k == 'q')
	exit(0);
}

static ushort gi_tgt_chg(ushort ct, ushort* tgt, ushort nv) {
   if (nv >= 0 && nv < ct) {
     if (*tgt != nv)
  	 *tgt = nv;
...
void gi_on_confirm(void* g, unsigned int k) {
  ...
  t = gi_tbl(g);
  tgt = &(t->selected);
  if (t->hover >=0 && t->hover < t->ct) {
   gi_tgt_chg(t->ct, tgt, t->hover);
  }
}
...
void gi_on_mw (void* g, int x, int y) {
}
