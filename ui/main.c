...
static GLfloat g_rotateAngle = 1;
static int g_Width = 600;                          // Initial window width
static int g_Height = 600;                         // Initial window height
static int g_yClick = 0;
static float g_lightPos[4] = { 10, 10, -10, 1 };  // Position of light
static GLuint *g_txNames = 0;
...
void RenderObjects(void)
{
  ...
  glBindTexture(GL_TEXTURE_2D, g_txNames[TEX_A]);

  DrawCubeWithTextureCoords(1.0);
  DrawCubeWithTextureCoords(2.0);
  DrawCubeWithTextureCoords(3.0);
...
{
   glGenTextures(8, g_txNames);
   int width, height;
   int nComponents;
   void* pTextureImage;
   texture_t* tex = textures[TEX_A];
...
int main(int argc, char** argv) {	
   ...
   g_txNames = calloc(argc, sizeof(GLuint));

   for(idx=1;idx<argc;idx++) {
	textures[idx] = texture_from_file(argv[idx]);
   }
   game.gi = gi_open(0,0, g_Width, g_Height, 9, 4);
